...
import { checkAndForbiddenOutOfDay, convertDateTime, noEditPartOfInput, timeInputsValidation } from './mainGlobFunctions';

// let merge = window._.merge;

export let renderCalendar = function renderCalendar(el: HTMLElement, option: any) {
  ...
  let options = merge(
    {
      ...
      // editable: true,
      selectable: true,
      select: function (info: { start: { getDate: () => any; }; end: { getDate: () => any; }; }) {
        // Проверяем, является ли выбранная дата или интервал дат заблокированным
        const isDateLocked = (date: { getDate: any; getMonth?: any; getFullYear?: any; }) => {
      ...
      viewDidMount: function (view: { view: { type: string; }; }) {
        const approveBtn = document.querySelector('.approveBtn') as HTMLElement;
        const lockBtn = document.querySelector('.lockBtn') as HTMLElement;
  ...
  const btnInView: keyof typeof btnViewObject = options.initialView;
  ...
  const dataViewTitle = document.querySelector('.data-view-title') as HTMLElement;
...
export const forceCalendarRecalculate = (cal: { prev: () => void; next: () => void; }) => {
  cal.prev();
  cal.next();
  cal.next();
  cal.prev();
